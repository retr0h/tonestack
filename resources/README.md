...
beside it. `schemas/embed.go` is that and nothing else, one exported symbol
each.
...
anybody hand-authors. The curated rigs moved to `pkg/sdk/rigs/`, beside the
library that reads them. Data nobody edits by hand lives in the package that
consumes it instead, which is what `pkg/catalog/data/`, `pkg/corpus/data/`,
`pkg/preset/data/` and `pkg/sdk/wire/data/` are.

## What may be redistributed

The curated rigs are ours, and they live in `pkg/sdk/rigs/` now.
