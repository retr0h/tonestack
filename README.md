...
| 🤖 [Your agent tunes it](docs/workflows.md#correct-a-rig-you-have-heard)       | Your agent builds a rig for the player you name, and rebuilds it each time you say what is wrong. The rig keeps every round, what you asked, what changed, why, and what you thought, so the next session starts from what worked                             |
| 🔌 Talks to your Helix                                                         | Reads, writes, copies, swaps and selects presets over USB, saving any slot to a file before it is overwritten. Device access is one backend per operating system, and macOS has the only one so far; building presets works everywhere. Tested on an HX Stomp |
| 🧩 [Go SDK](CONTRIBUTING.md#what-to-import-if-you-are-using-this-as-a-library) | `pkg/sdk` is the library the CLI is built on: build presets from rigs, read and write a device, and look up what it can do, from your own Go program                                                                                                          |
